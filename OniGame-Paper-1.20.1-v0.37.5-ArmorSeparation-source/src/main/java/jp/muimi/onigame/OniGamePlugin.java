...
public final class OniGamePlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    @Override public void onEnable(){saveDefaultConfig(); String forcedOniRaw=getConfig().getString("forced-oni-uuid","");if(forcedOniRaw!=null&&!forcedOniRaw.isBlank())try{forcedOni=UUID.fromString(forcedOniRaw);}catch(IllegalArgumentException ignored){forcedOni=null;} actionKey=new NamespacedKey(this,"action"); getServer().getPluginManager().registerEvents(this,this); PluginCommand c=getCommand("onigame"); c.setExecutor(this); c.setTabCompleter(this); restoreLobbyUtilities();refreshLobbyGuideDisplays();sidebarTicker=Bukkit.getScheduler().runTaskTimer(this,this::updateSidebars,1,20); chaseTicker=Bukkit.getScheduler().runTaskTimer(this,this::updateHorrorState,5,5);repairTicker=Bukkit.getScheduler().runTaskTimer(this,this::updatePlayerActions,2,2);staminaTicker=Bukkit.getScheduler().runTaskTimer(this,this::updateStamina,5,5);botTicker=Bukkit.getScheduler().runTaskTimer(this,this::updateBots,10,10);getLogger().info("鬼げぇむ v0.37.5 enabled");}
    ...
    private int oniSkillUnlockHearts(String action){
        return switch(action){
            ...
            case "shikki_hunting_leap"->getConfig().getInt("shikki-skills.hunting-leap.unlock-after-broken-hearts",4);
    ...
    private void refreshOniUnlockedSkills(Player p,boolean announce){
        ...
        if(oniType==OniType.DAKKO){
            ...
            if(oniSkillUnlocked("shikki_hunting_leap")&&p.getInventory().getItem(3)==null){p.getInventory().setItem(3,oniSkillIconItem("shikki_hunting_leap","&6&l跳躍狩り","shikki_hunting_leap"));if(announce)msg(p,"&6跳躍狩り &fが解放された！");}
    ...
    @Override public boolean onCommand(CommandSender sender,Command command,String label,String[] a){
        ...
        if(a[0].equalsIgnoreCase("version")){msg(sender,"&dOniGame Runtime &f0.37.5 &7| GUI schema: &fskills 2p / passives 4p &7| Expected RP: &fv37");return true;}
    ...
    private void start(OniType forced,CommandSender starter,boolean playerTest){
        ...
        OniType upcomingOniType=forced!=null?forced:(selectedOniType!=null?selectedOniType:new OniType[]{OniType.DAKKO,OniType.KISHIN,OniType.SHIKKI,OniType.YUUKI}[new Random().nextInt(4)]);
        ...
        int requiredRealHearts=oniType==OniType.JAKUTSUKI?Math.max(1,getConfig().getInt("jakutsuki-hearts.real-count",8)):Math.max(1,getConfig().getInt("random-heart-count",8));
    ...
    private OniType requireOniType(CommandSender sender,String raw){
        ...
        if(type==null){msg(sender,"&c不明な鬼タイプ: &f"+raw+" &7(dakko / kishin / shikki / yuuki / jakutsuki)");return null;}
    ...
    private int oniSkillIconModel(String action){return switch(action){
        ...
        case "shikki_frenzy"->3431;case "shikki_blood_run"->3432;case "shikki_hunting_leap"->3433;case "shikki_chain_hunt"->3434;
    ...
    private void openOniMenu(Player p){
        ...
        if(isJakutsukiLocked()){menu.setItem(12,customModelItem(Material.CHAINMAIL_HELMET,1001,"&5&l堕狐","menu_oni:DAKKO","&7奇襲・攪乱型","&d狐渡り &7/ &6分霊 &7/ &6狐火 &7/ &d天来",shown==OniType.DAKKO?"&a現在表示・選択中":"&eクリックして選択"));menu.setItem(14,menuItem(Material.NETHERITE_AXE,"&4&l鬼神","menu_oni:KISHIN","&7正面突破・追跡型","&c鬼突 &7/ &4地砕 &7/ &4鬼吼 &7/ &6剛身",shown==OniType.KISHIN?"&a現在表示・選択中":"&eクリックして選択"));menu.setItem(16,customModelItem(Material.CARVED_PUMPKIN,1201,"&c&l疾鬼","menu_oni:SHIKKI","&7索敵・高速機動・連続狩猟型","&c狂奔 &7/ &c血走 &7/ &6跳躍狩り &7/ &4狩猟連鎖",shown==OniType.SHIKKI?"&a現在表示・選択中":"&eクリックして選択"));menu.setItem(10,customModelItem(Material.CARVED_PUMPKIN,1202,"&8&l幽鬼","menu_oni:YUUKI","&7隠密・奇襲型","&8幽歩 &7/ &7朧渡り &7/ &5影縫い &7/ &8神隠し",shown==OniType.YUUKI?"&a現在表示・選択中":"&eクリックして選択"));}
        else{menu.setItem(11,customModelItem(Material.CHAINMAIL_HELMET,1001,"&5&l堕狐","menu_oni:DAKKO","&7奇襲・攪乱型","&d狐渡り &7/ &6分霊 &7/ &6狐火 &7/ &d天来",shown==OniType.DAKKO?"&a現在表示・選択中":"&eクリックして選択"));menu.setItem(13,customModelItem(Material.PLAYER_HEAD,2009,"&8&lEX鬼 蛇窟姫 &7(ジャクツキ)","menu_oni:JAKUTSUKI","&7領域制圧・終盤強化型","&5黒鏡 &7/ &5薙ぎ払い &7/ &8蛇崩 &7/ &4解放 &7/ &5黒の波動","&c基礎体力200 / 心臓破壊ごとに最大体力-10","&c固有ルール: 鬼の心臓が8個に増加","&4偽心臓4個 &7――接近すると爆発","&7爆発は地形を破壊せず、ダメージ＋発光を付与","&f全心臓破壊時: 全ぷれいやーへ『蛇斬』支給","&6救援: 残存4人以下 または 心臓残り3個でアザクジ参戦",shown==OniType.JAKUTSUKI?"&a現在表示・選択中":"&eクリックして選択"));menu.setItem(15,menuItem(Material.NETHERITE_AXE,"&4&l鬼神","menu_oni:KISHIN","&7正面突破・追跡型","&c鬼突 &7/ &4地砕 &7/ &4鬼吼 &7/ &6剛身",shown==OniType.KISHIN?"&a現在表示・選択中":"&eクリックして選択"));menu.setItem(17,customModelItem(Material.CARVED_PUMPKIN,1201,"&c&l疾鬼","menu_oni:SHIKKI","&7索敵・高速機動・連続狩猟型","&c狂奔 &7/ &c血走 &7/ &6跳躍狩り &7/ &4狩猟連鎖",shown==OniType.SHIKKI?"&a現在表示・選択中":"&eクリックして選択"));menu.setItem(9,customModelItem(Material.CARVED_PUMPKIN,1202,"&8&l幽鬼","menu_oni:YUUKI","&7隠密・奇襲型","&8幽歩 &7/ &7朧渡り &7/ &5影縫い &7/ &8神隠し",shown==OniType.YUUKI?"&a現在表示・選択中":"&eクリックして選択"));}
        if(shown==OniType.DAKKO){menu.setItem(20,menuItem(Material.ENDER_PEARL,"&d&l狐渡り","menu_decor","&7視線先へ瞬間移動","&7心臓破壊で射程低下・CT増加"));menu.setItem(24,menuItem(Material.FOX_SPAWN_EGG,"&6&l分霊","menu_decor","&7弱体化した堕狐鬼Botの分身体を2体召喚","&7約5秒だけ追跡・攻撃して消滅","&7心臓2個破壊で解放"));menu.setItem(29,menuItem(Material.BLAZE_POWDER,"&6&l狐火","menu_decor","&7周囲へ固定ダメージ＋炎上","&7対集団用 / 心臓4個破壊で解放"));menu.setItem(33,menuItem(Material.FIREWORK_STAR,"&d&l天来","menu_decor","&7視線先の上空から飛来して範囲攻撃","&7心臓6個破壊で解放"));}
        else if(shown==OniType.KISHIN){menu.setItem(20,menuItem(Material.FIREWORK_ROCKET,"&c&l鬼突","menu_decor","&7前方へ強力な突進","&7心臓破壊で速度低下・CT増加"));menu.setItem(22,menuItem(Material.IRON_AXE,"&4&l地砕","menu_decor","&7周囲を攻撃し、吹き飛ばして鈍足付与","&7心臓2個破壊で解放"));menu.setItem(24,menuItem(Material.GOAT_HORN,"&4&l鬼吼","menu_decor","&7咆哮で周囲を攻撃し、鈍足＋弱体化","&c威力4 / 半径8m / CT28秒","&7心臓4個破壊で解放"));menu.setItem(31,menuItem(Material.SHIELD,"&6&l剛身","menu_decor","&76秒間、耐性I＋移動速度上昇II＋攻撃力上昇II","&7追跡・接近戦を大きく強化 / CT35秒","&7心臓6個破壊で解放"));}else if(shown==OniType.SHIKKI){menu.setItem(20,oniSkillIconItem("shikki_frenzy","&c&l狂奔","menu_decor","&7約8秒間、高速追跡モードへ移行","&7狂奔中の通常攻撃威力は低下","&7命中すると別の獲物を索敵 / CT32秒"));menu.setItem(22,oniSkillIconItem("shikki_blood_run","&c&l血走","menu_decor","&7周囲の走行中・負傷中のぷれいやーを感知","&7方向と距離を表示 / 心臓2個破壊で解放"));menu.setItem(24,oniSkillIconItem("shikki_hunting_leap","&6&l跳躍狩り","menu_decor","&7前方へ高速跳躍して障害物を越える","&7狂奔中は飛距離増加 / 心臓4個破壊で解放"));menu.setItem(31,oniSkillIconItem("shikki_chain_hunt","&4&l狩猟連鎖","menu_decor","&7狂奔中、異なる獲物への連続命中で強化","&73人連鎖でフィニッシュ攻撃を準備","&7心臓6個破壊で解放"));}else if(shown==OniType.YUUKI){menu.setItem(20,oniSkillIconItem("yuuki_veil","&8&l幽歩","menu_decor","&7透明化＋足音抑制＋移動速度上昇","&7透明中は攻撃不可。再使用で解除し奇襲加速"));menu.setItem(22,oniSkillIconItem("yuuki_haze_step","&7&l朧渡り","menu_decor","&7幽歩中、前方へ短距離高速移動","&7壁抜け不可 / 心臓2個破壊で解放"));menu.setItem(24,oniSkillIconItem("yuuki_shadow_bind","&5&l影縫い","menu_decor","&7幽歩解除時、近距離の獲物を拘束","&7鈍足＋短時間スキル封印 / 心臓4個破壊で解放"));menu.setItem(31,oniSkillIconItem("yuuki_divine_hide","&8&l神隠し","menu_decor","&7幽歩を強化し、奇襲性能を上昇","&7解除後の初撃強化 / 心臓6個破壊で解放"));}else{menu.setItem(20,menuItem(Material.BLACK_STAINED_GLASS,"&5&l黒鏡","menu_decor","&7SHIFT+右クリックで鏡を設置","&7右クリックで設置地点へ転移 / 長CT"));menu.setItem(22,menuItem(Material.NETHERITE_HOE,"&5&l薙ぎ払い","menu_decor","&7前方の扇状範囲を一閃"));menu.setItem(24,menuItem(Material.SILVERFISH_SPAWN_EGG,"&8&l蛇崩","menu_decor","&7大量の蛇を一定時間召喚","&7蛇に噛まれると短時間発光"));menu.setItem(29,menuItem(Material.ECHO_SHARD,"&d&l黒の波動","menu_decor","&7溜めの後、前方へ強力な貫通範囲攻撃","&7複数のぷれいやーを一直線に貫く","&c威力16 / 射程28m / 溜め2.5秒"));menu.setItem(31,menuItem(Material.NETHER_STAR,"&4&l解放","menu_decor","&7心臓が残り1つの時のみ使用可能","&7即狂化＋脱出口を60秒封印"));menu.setItem(33,menuItem(Material.CRYING_OBSIDIAN,"&4&l蛇窟の偽心臓","menu_decor","&7本物8個＋偽物4個が配置される","&7偽物へ接近すると爆発","&cダメージ＋発光 &7/ 地形破壊なし","&7探知機・霊視でも本物と区別できない"));}
    ...
    private void equipShikkiArmor(Player p){p.getInventory().setHelmet(dyedOniArmor(Material.LEATHER_HELMET,org.bukkit.Color.fromRGB(32,28,28),"&c疾鬼の兜",1210));p.getInventory().setChestplate(dyedOniArmor(Material.LEATHER_CHESTPLATE,org.bukkit.Color.fromRGB(105,12,18),"&4疾鬼の装束",1211));p.getInventory().setLeggings(dyedOniArmor(Material.LEATHER_LEGGINGS,org.bukkit.Color.fromRGB(32,28,28),"&8疾鬼の袴",1212));p.getInventory().setBoots(dyedOniArmor(Material.LEATHER_BOOTS,org.bukkit.Color.fromRGB(120,18,22),"&4疾鬼の足具",1213));}
    ...
    private void equipShikkiArmor(LivingEntity e){e.getEquipment().setHelmet(dyedOniArmor(Material.LEATHER_HELMET,org.bukkit.Color.fromRGB(32,28,28),"&c疾鬼の兜",1210));e.getEquipment().setChestplate(dyedOniArmor(Material.LEATHER_CHESTPLATE,org.bukkit.Color.fromRGB(105,12,18),"&4疾鬼の装束",1211));e.getEquipment().setLeggings(dyedOniArmor(Material.LEATHER_LEGGINGS,org.bukkit.Color.fromRGB(32,28,28),"&8疾鬼の袴",1212));e.getEquipment().setBoots(dyedOniArmor(Material.LEATHER_BOOTS,org.bukkit.Color.fromRGB(120,18,22),"&4疾鬼の足具",1213));}
    ...
    private void renderOniHeartGlow(){
        if(state!=GameState.RUNNING||oni==null){clearOniHeartGlowMarkers();return;}
        Player viewer=Bukkit.getPlayer(oni);if(viewer==null||!viewer.isOnline())return;
        ...
        for(String key:heartHp.keySet()){
            ...
            for(Player other:Bukkit.getOnlinePlayers()){
                if(other.getUniqueId().equals(oni))other.showEntity(this,marker);
                else other.hideEntity(this,marker);
            }
            if(heart.getWorld().equals(viewer.getWorld())){
    ...
    private void updateOniBot(){
        ...
        if(oniType==OniType.DAKKO){
            ...
            int ticks=Math.max(20,getConfig().getInt("shikki-skills.frenzy.duration-ticks",160));bot.addPotionEffect(new PotionEffect(PotionEffectType.SPEED,ticks,Math.max(0,getConfig().getInt("shikki-skills.frenzy.speed-amplifier",2)),false,true));bot.getWorld().spawnParticle(Particle.CRIT,bot.getLocation().add(0,1,0),30,.4,.7,.4,.06);botNextAbilityAt=now+oniBotAbilityCooldownMillis(getConfig().getInt("shikki-skills.frenzy.cooldown-seconds",32));recoverOniSkillMomentum();return;
        }
    ...
    private boolean isOniSkillAction(String id){
        return id!=null&&(id.startsWith("dakko_")||id.startsWith("kishin_")||id.startsWith("shikki_")||id.startsWith("yuuki_")||id.startsWith("jakutsuki_"));
    }
    ...
    private void useShikkiFrenzy(Player p){
        ...
        p.sendTitle("",cc("&c&l狂 奔 &7――獲物を連鎖せよ"),0,30,8);
    }
    ...
    private void useShikkiHuntingLeap(Player p){
        if(!ready(p,"shikki_hunting_leap",getConfig().getInt("shikki-skills.hunting-leap.cooldown-seconds",22)))return;
        ...
        Vector v=p.getLocation().getDirection().normalize().multiply(power);v.setY(Math.max(.48,v.getY()+.48));p.setVelocity(v);p.setFallDistance(0);p.getWorld().playSound(p.getLocation(),Sound.ENTITY_ENDER_DRAGON_FLAP,.8f,1.55f);p.getWorld().spawnParticle(Particle.CLOUD,p.getLocation(),24,.35,.15,.35,.07);
    }
    private void useShikkiChainHunt(Player p){
        ...
        int extend=Math.max(20,getConfig().getInt("shikki-skills.chain-hunt.extend-ticks",80));shikkiFrenzyUntil+=extend*50L;p.addPotionEffect(new PotionEffect(PotionEffectType.SPEED,(int)Math.max(1,(shikkiFrenzyUntil-System.currentTimeMillis())/50),3,false,true));p.sendActionBar(cc("&4狩猟連鎖 &7――狂奔延長・速度上昇IV"));
    }
    ...
    @EventHandler public void onDamage(EntityDamageByEntityEvent e){
        ...
        if(attackerIsOni&&rawAttacker instanceof Player shikkiHunter&&e.getEntity() instanceof Player shikkiVictim&&players.contains(shikkiVictim.getUniqueId()))onShikkiFrenzyHit(shikkiHunter,shikkiVictim,e);
